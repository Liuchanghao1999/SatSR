...
MBT = 4.0
DNN_SIZE = 4302035 # SR model size, bytes
SMOOTH_PENALTY = 1
...
NOISE_LOW = 0.9
NOISE_HIGH = 1.1
SR_TIME_FILE = ['./iPHONE_GPU_4_', './challenge_GPU_4_', './CatBox_GPU_4_', './favorite_GPU_4_', './PS4_GPU_4_']
...
VIDEO_CHUNK_NUM = [86, 202, 176, 180, 179] # Number of chunks in each video
...
class Environment:
    def __init__(self, all_cooked_user_time, all_cooked_user_bw, all_cooked_sat_time, all_cooked_sat_bw, random_seed=RANDOM_SEED):
        ...
        self.video_idx = np.random.randint(5)
        ...
        self.sr_delay[0] = []
        self.sr_delay[1] = []
        with open(SR_TIME_FILE[self.video_idx] + str(480) + '.log') as f:
            for line in f:
                self.sr_delay[1].append(float(line))
        self.sr_delay[2] = []
        with open(SR_TIME_FILE[self.video_idx] + str(720) + '.log') as f:
            for line in f:
                self.sr_delay[2].append(float(line))
        self.sr_delay[3] = []
        with open(SR_TIME_FILE[self.video_idx] + str(1080) + '.log') as f:
            for line in f:
                self.sr_delay[3].append(float(line))
        assert len(self.sr_delay[1]) == len(self.sr_delay[2]) == len(self.sr_delay[3])
        for i in range(len(self.sr_delay[1])):
            self.sr_delay[0].append(0.0)
        ...
        while self.last_trans_dnn_counter_sent < DNN_SIZE:
            ...
            while True:  # download HD-video chunk over sat-link
                if self.sat_bw[self.mahimahi_ptr_sat] < 0.2:
                    throughput = self.sat_bw[self.mahimahi_ptr_sat - 1] * B_IN_MB / BITS_IN_BYTE
                else:
                    throughput = self.sat_bw[self.mahimahi_ptr_sat] * B_IN_MB / BITS_IN_BYTE
                duration = self.sat_time[self.mahimahi_ptr_sat] - self.last_mahimahi_time_sat
                packet_payload = throughput * duration * PACKET_PAYLOAD_PORTION

                if video_chunk_counter_sent + packet_payload > video_chunk_size_hd:

                    fractional_time = (video_chunk_size_hd - video_chunk_counter_sent) / throughput / PACKET_PAYLOAD_PORTION
            ...
            # Make sure the time schedule of sat and user trace is synchronized
            ...
            delay_rand = np.random.uniform(NOISE_LOW, NOISE_HIGH)
            delay *= delay_rand
            user_delay *= delay_rand
            ...
            if self.buffer_size >= 10.0:
                bola_size = min(self.user_video_chunk_counter - 1, self.video_chunk_num - self.user_video_chunk_counter + 1)
